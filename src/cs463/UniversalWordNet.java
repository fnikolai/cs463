...
public class UniversalWordNet {
    ...
    public UniversalWordNet() throws Exception {
        ...
        //String plugins = UniversalWordNet.class.getResource("plugins/").getPath();
        uwn = new UWN(new File("plugins/"));
    }

    // The ISO 639-3 code for Greek is "ell". For English use "eng". Please refer to
    ...
    public void exampleUsageOfUWN(String term) throws IOException {
        ...
        // rel:means (synset, i.e. the set of terms that have the same meaning) e.g. s/n7212190
        // rel:has_derived_form
        // rel:etymological_origin_of
        // rel:etymology
        // rel:is_derived_from
        // rel:lexicalization (i.e. different lexicalizations for different languages)
        // rel:similar
        // rel:closely_related
        // rel:opposite
        ...
        while (it.hasNext()) {
            ...
            if (predicate.getId().equals("rel:means")) {
                ...
                while (meanings.hasNext()) {
                    ...
                    // Print only greek and english related words for these meanings
                    if (predicateOfMeaning.getId().equals("rel:lexicalization") && (objectOfMeaning.getTermLanguage().equals("ell") || objectOfMeaning.getTermLanguage().equals("eng"))) {
                ...
                while (meanings.hasNext()) {
                    ...
                    if (!(predicateOfMeaning.getId().equals("rel:lexicalization"))) {
                        System.out.println("\tEntity: " + subjectOfMeaning.getId() + " Relation " + predicateOfMeaning.getId() + " :\"" + objectOfMeaning.getId() + "\" with weight " + stmt.getWeight());
                    }
                }
            }
            System.out.println();
